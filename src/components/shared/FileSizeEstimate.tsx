...
export function FileSizeEstimate({ originalSize, compressionLevel, locale = 'en' }: FileSizeEstimateProps) {
  ...
  const rangeText = `${formatFileSize(estimate.min)} – ${formatFileSize(estimate.max)}`;
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-gray-400">
          ...
          <span className="text-sm">{t('Preliminary estimate', 'تقدير مبدئي')}</span>
        </div>
        <span className="text-violet-400 font-medium">{formatFileSize(estimate.expected)}</span>
      </div>

      <div className="flex items-center justify-between">
        <span className="text-sm text-gray-500">{t('Expected range', 'النطاق المتوقع')}</span>
        <span className="text-sm text-gray-400">{rangeText}</span>
      </div>
      ...
      <div className="flex items-center justify-between pt-2 border-t border-gray-700">
        ...
        <div className="text-center">
          <p className="text-xs text-gray-500">{t('After (estimate)', 'بعد (تقدير)')}</p>
      ...
      <p className="text-[11px] text-gray-500">
        {t(
          'Final size may differ depending on the file content. The exact result appears after processing.',
          'قد يختلف الحجم النهائي حسب محتوى الملف. تظهر النتيجة الدقيقة بعد انتهاء المعالجة.'
        )}
      </p>
    </motion.div>
  );
