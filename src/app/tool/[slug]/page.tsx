...
export default function ToolPage() {
  ...
  const compressPdfReal = async (
    ...
    // Adaptive scaling for large PDFs (faster + lower memory)
    try {
      const pdfForCount = await PDFDocument.load(buffer, { ignoreEncryption: true });
      const pageCount = pdfForCount.getPageCount();
      // Reduce scale progressively as page count grows
      ...
      else if (pageCount >= 20) settings = { ...settings, scale: Math.min(settings.scale, 1) };
      // Fast mode forces a lower rendering scale
      if (fastMode) settings = { ...settings, scale: Math.min(settings.scale, 0.85) };
    } catch {
      // If we can't read page count, continue with defaults
    }
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      try {
        ...
        const compressed = await imageCompression(file, {
          ...
          maxWidthOrHeight: settings.scale > 1.1 ? 4096 : 2048,
          useWebWorker: true,
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      // PDF: Compress (real compression - converts to images, compresses, converts back)
        ...
        // Prefer running in a Web Worker to avoid blocking the UI.
        // Fallback to main-thread implementation if worker path isn't supported.
        let pdfBytes: Uint8Array;
        try {
          pdfBytes = await compressPdfWithWorker(
            buffer,
            toolOptions.compressionLevel || 'medium',
            !!toolOptions.fastMode,
            (p) => setProcessingProgress(p)
          );
        } catch {
          // Worker failed (e.g., OffscreenCanvas unsupported). Use main-thread fallback.
          // Note: buffer might have been transferred to the worker; re-read if needed.
          if (buffer.byteLength === 0) buffer = await files[0].file.arrayBuffer();
          pdfBytes = await compressPdfReal(
            buffer,
            toolOptions.compressionLevel || 'medium',
            !!toolOptions.fastMode,
            (p) => setProcessingProgress(p)
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 'done' && (
              <motion.div key="done" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="bg-gray-900/50 rounded-2xl border border-gray-800 p-8 sm:p-12">
                      ...
                      <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-gray-800/50 border border-gray-700">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <span className="text-gray-300 text-sm truncate block">{file.name}</span>
                            {typeof file.originalSize === 'number' && typeof file.processedSize === 'number' && (
                              <span className="text-xs text-gray-500 block">
                                {formatFileSize(file.originalSize)} → {formatFileSize(file.processedSize)}
                                {typeof file.savingsPercent === 'number' && (
                                  <span className="text-green-400"> {' '}({file.savingsPercent > 0 ? `-${file.savingsPercent}%` : '0%'})</span>
                                )}
                              </span>
                            )}
                          </div>
                ...
                {processedFiles.length === 1 && typeof processedFiles[0]?.originalSize === 'number' && typeof processedFiles[0]?.processedSize === 'number' && (
                  <div className="mb-6 p-4 rounded-xl bg-gray-800/40 border border-gray-700 text-center">
                    <p className="text-sm text-gray-300">
                      {formatFileSize(processedFiles[0].originalSize!)} → {formatFileSize(processedFiles[0].processedSize!)}
                      {typeof processedFiles[0].savingsPercent === 'number' && (
                        <span className="text-green-400"> {' '}({processedFiles[0].savingsPercent! > 0 ? `-${processedFiles[0].savingsPercent}%` : '0%'})</span>
                      )}
                    </p>
                    <p className="text-xs text-gray-500 mt-1">
                      {isRtl ? 'هذه هي النتيجة الفعلية بعد المعالجة.' : 'This is the actual result after processing.'}
                    </p>
                  </div>
                )}
